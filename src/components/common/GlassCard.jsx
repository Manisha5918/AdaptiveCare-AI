...
export const GlassCard = ({ 
  ...
  // Safely retrieve density from context
  let density = 'balanced';
  try {
    const app = useApp();
    if (app) density = app.density;
  } catch (e) {
    // context not active
  }

  const getDensityClasses = (d) => {
    switch (d) {
      case 'simple': return 'p-8 sm:p-10 rounded-[28px]';
      case 'detailed': return 'p-4 sm:p-5 rounded-[20px]';
      default: return 'p-6 sm:p-7 rounded-[24px]'; // balanced
    }
  };
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className={`glass-panel relative overflow-hidden transition-all duration-300 group ${getDensityClasses(density)} ${onClick ? 'cursor-pointer' : ''} ${className}`}
      {...props}
    >
